...
use foundry_compilers::{
    artifacts::{remappings::Remapping, BytecodeObject, ContractBytecodeSome, Libraries, Source}, compilers::{
        solc::{Solc, SolcCompiler},
        Compiler,
    }, multi::MultiCompilerLanguage, report::{BasicStdoutReporter, NoReporter, Report}, solc::SolcSettings, zksync::{
...
use std::{
    collections::{BTreeMap, HashMap, HashSet}, fmt::Display, io::IsTerminal, path::{Path, PathBuf}, sync::Arc, time::Instant
};
    ...
    fn zksync_handle_output(
        ...
        let missing_libs: Vec<ZkMissingLibrary> = missing_libs_unique
            ...
            .map(|ml| {
                ...
                let abs_path_str = abs_path_buf.to_string_lossy().into_owned();

                let art = output.find(Path::new(&abs_path_str), contract_name).unwrap_or_else(|| {
                    panic!(
